...
CsmaNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  //std::cout << "packet size after ether " << packet->GetSize() << " " << protocolNumber << std::endl;
  ...
  if (m_queue->Enqueue (packet) == false)
    {
      m_macTxDropTrace (packet);
      /* One problem we have in our simulations is that congestion actually happens at
      /* the egress port, so this might be problematic for all the detection systems */
      NS_LOG_UNCOND("Packet Dropped: " << m_queue->GetMaxSize());
      return false;
    }
