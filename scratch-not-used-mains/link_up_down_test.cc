...
main (int argc, char *argv[])
{
  ...
  devices1 = pointToPoint.Install (NodeContainer(nodes.Get(1), nodes.Get(2)));
...
//  devices.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
  ...
  app->Setup (ns3TcpSocket, sinkAddress, 1040, 3500, DataRate ("1Mbps"));
  ...
  pointToPoint.EnablePcap("out", devices.Get(0), bool(1));

  Simulator::Schedule(Seconds(2), &LinkDown, devices1);
  Simulator::Schedule(Seconds(10), &LinkUp, devices1);

  Simulator::Schedule(Seconds(20), &LinkDown, devices1);
  //Simulator::Schedule(Seconds(22), &LinkUp, devices1);
